...
function Entertainment({ categorieName }) {
                ...
                <div className='flex items-center justify-between'>
                  <div>
                    <div className='flex items-center gap-1'>
                      ...
                      <h5 className='text-sm text-gray-800 my-2'>{course.duration}</h5>
                    </div>
                    <div className='flex items-center gap-1'>
                      <CiSquarePlus className='text-sky-400' size={25} />
                      <h5 className='text-sm text-gray-800'>{course.level}</h5>
                    </div>
                  </div>
                  <Link to={`/entertainmentdetails/${course.id}`}><button className='btn rounded-xl px-2 outline-none  bg-white border-gray-400 text-gray-600 text-sm mt-4 hover:text-white hover:bg-rose-500 hover:border-none'>বিস্তারিত পড়ুন
                  </button></Link>
                </div>
              </div>
